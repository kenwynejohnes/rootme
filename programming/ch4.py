...
import socket, zlib #connect special library socket
...
def ep4_solve():
    bot.send("PRIVMSG Candy :!ep4\n") #send message to Candy Bot
    smsg = ""
    smsg = bot.recv(7000)
    print(smsg)
    smsg=smsg[42:smsg.find(":")-1]
    smsg = str.decode(smsg, 'base64')
    smsg = zlib.decompress(smsg)
    bot.send("PRIVMSG Candy :!ep4 -rep " + smsg + " \n")
    print("Answer sent")
    print(bot.recv(7000))
